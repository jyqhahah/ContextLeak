...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def target_mode(self):
        ...
        if hasattr(peft_model, "peft_config"):
            ...
            params = {replace_lora_wrapper(k, peft_model.peft_config.get("default", None)): v 
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="blue", role="target_generate")
    def target_generate_sequences(self, prompts: DataProto):
        prompts = prompts.to(get_device_id())
        data = prompts
        ...
        if self._is_actor:
            loop = get_event_loop()
            loop.run_until_complete(self.target_mode())      
            log_gpu_memory_usage("After switch to target mode", logger=logger)

        with simple_timer("target_generate", timing_generate):
            output = self.rollout.target_generate_sequences(prompts=data)

        if self._is_actor:
            loop.run_until_complete(self.trainer_mode())   
            log_gpu_memory_usage("After switch back to trainer mode", logger=logger)

        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["target_generate"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update({
            "generation_timing/max": timing_generate_max,
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="rollout"))
    @DistProfiler.annotate(color="blue", role="target_generate")
    def summarizer_generate_sequences(self, prompts: DataProto):
        prompts = prompts.to(get_device_id())
        data = prompts
        ...
        with simple_timer("summarizer_generate", timing_generate):
            output = self.rollout.summarizer_generate_sequences(prompts=data)
        ...
        timing_generate_topk_ratio, timing_generate_min, timing_generate_max = topk_reduce_ratio_min_max(
            timing_generate["summarizer_generate"]
        )
        timing_generate = reduce_timing(timing_generate)
        timing_generate.update({
            "generation_timing/max": timing_generate_max,
