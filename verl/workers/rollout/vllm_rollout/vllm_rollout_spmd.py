...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # self.lora_kwargs = (
        #     {"enable_lora": True, "max_loras": 2, "max_lora_rank": get_vllm_max_lora_rank(lora_rank)}
        #     if model_config.lora_rank > 0
        #     else {}
        # )
        ...
        self.actor_lora_id = None
        self._next_lora_id = 2
        kwargs = dict(
            n=1,
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        print(f"vllm_rollout_spmd.py line 397: [generate_sequences] do_sample={do_sample}, is_validate={is_validate}")
        print(f"vllm_rollout_spmd.py line 398: [generate_sequences] kwargs={kwargs}")
        ...
        # lora_requests = None
        # if self.lora_kwargs and self.actor_lora_id is not None:
        #     lora_requests = [
        #         LoRARequest(
        #             lora_name=str(self.actor_lora_id),
        #             lora_int_id=self.actor_lora_id,
    ...
    @GPUMemoryLogger(role="vllm target spmd", logger=logger)
    @torch.no_grad()
    def target_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for input_data in vllm_inputs:
            if not isinstance(input_data["prompt_token_ids"], (list, np.ndarray)):
                raise TypeError(
                    f"prompt_token_ids must be list or numpy array, got {type(input_data['prompt_token_ids'])}"
                )
            input_data["prompt_token_ids"] = list(input_data["prompt_token_ids"])
        ...
        # ⭐ top-100 logprobs
        with self.update_sampling_params(logprobs=5, **kwargs):
            outputs = self.inference_engine.generate(
                prompts=vllm_inputs,
                sampling_params=self.sampling_params,
                lora_request=lora_requests,
            ...
            response = []
            # ⭐ 存每个位置的 top-100 logprob dict，用 non_tensor_batch 传递
            all_logprobs = []
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    # 每个位置存 {token_id: logprob} 的 dict
                    pos_logprobs = []
                    if output.outputs[sample_id].logprobs is not None:
                        for logprob_dict in output.outputs[sample_id].logprobs:
                            # logprob_dict: {token_id: Logprob}
                            pos_logprobs.append(
                                {tid: lp.logprob for tid, lp in logprob_dict.items()}
                            )
                    else:
                        pos_logprobs = [{} for _ in range(len(response_ids))]
        ...
        response_length = response.size(1)
        delta_position_id = torch.arange(1, response_length + 1, device=position_ids.device).unsqueeze(0)
        ...
        batch = TensorDict(
            {
                ...
                "input_ids_target": seq,
                "attention_mask_target": attention_mask,
                "position_ids_target": position_ids,
            },
            batch_size=batch_size,
        )

        # ⭐ logprobs 用 non_tensor_batch 传递，因为是变长的嵌套结构
        non_tensor_batch["target_logprobs"] = np.array(all_logprobs, dtype=object)

        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch)

    @GPUMemoryLogger(role="vllm target spmd", logger=logger)
    @torch.no_grad()
    def summarizer_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        idx = prompts.batch["input_ids"]
        attention_mask = prompts.batch["attention_mask"]
        position_ids = prompts.batch["position_ids"]
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        if peft_config and base_sync_done:
            ...
            # self.actor_lora_id = lora_int_id
            lora_tensors = dict(weights)
            print(f"vllm_rollout_spmd.py line 720: [update_weights] add_lora id={lora_int_id}, num_tensors={len(lora_tensors)}")
            ...
            print(f"vllm_rollout_spmd.py line 730: [update_weights] registered after add: {self.inference_engine.llm_engine.list_loras()}")
            logger.info(f"vLLM load weights, loaded_params: {len(weights)}")
        else:
            print("vllm_rollout_spmd.py line 749: [update_weights] branch=load_base_weights")
