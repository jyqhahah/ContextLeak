...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def _init_strategy_library(self):
        """
        Initialize three separate strategy pools:
          - tool_failure:   failed to select malicious tool
          - low_similarity: tool selected but argument quality low
          - full_success:   tool selected and argument quality high
        Each pool holds up to pool_max_size entries (default 100).
        """
        self.library = {
            "tool_failure":   {},
            "low_similarity": {},
        ...
        if self.use_strategy:
            ...
            self._strategy_embedding_model = SentenceTransformer("all-MiniLM-L6-v2").to("cpu")
            print(f"[Strategy] Library initialized. pool_max_size={self.pool_max_size} per pool.")
        else:
            self._strategy_embedding_model = None
            print("[Strategy] Library disabled (use_strategy=False).")

    def _save_strategy_library(self):
        """保存 strategy library 到 checkpoint 目录。"""
        if not self.use_strategy or not any(self.library.values()):
            return
        import pickle
        ...
        try:
            ...
            sizes = {pt: len(p) for pt, p in self.library.items()}
    ...
    def _load_strategy_library(self):
        ...
        try:
            ...
            self._library_version = saved.get("library_version", 0)
            sizes = {pt: len(p) for pt, p in self.library.items()}
            ...
            # rebuild FAISS cache from loaded library
            self._build_faiss_cache()
        except Exception as e:
            print(f"[Strategy] Failed to load library: {e}")

    def _build_faiss_cache(self):
        """Rebuild FAISS index cache for all pools. Called only when library changes."""
        import faiss
        self._faiss_cache = {}
        for pool_type, pool in self.library.items():
            if not pool:
                continue
            # each item in these lists corresponds to one example (not one strategy)
            all_embeddings  = []
            all_descriptions = []
            all_definitions  = []
            all_strategy_names = []

            for strategy_name, entry in pool.items():
                for ex in entry.get("examples", []):
                    emb = ex.get("embedding", None)
                    if emb is None:
                        continue
                    all_embeddings.append(np.array(emb, dtype=np.float32))
                    all_descriptions.append(ex.get("description", ""))
                    all_definitions.append(ex.get("definition", ""))
        ...
        sizes = {pt: len(c["descriptions"]) for pt, c in self._faiss_cache.items()}
        print(f"[Strategy] FAISS cache rebuilt. Sizes (examples): {sizes}")

    def _retrieve_strategies_batch(self, query_embeddings: np.ndarray, is_val: bool = False) -> list:
        """
        Batch FAISS retrieval using cached index (rebuilt only when library changes).
        Each retrieved entry has its own definition (example-level, not strategy-level).
        Returns list of strategy_lists, one per query.
        Fixed retrieval counts: 1 tool_failure + 1 low_similarity + 2 full_success.
        """
        ...
        for pool_type, n in counts.items():
            if pool_type not in self._faiss_cache:
                continue
            cache          = self._faiss_cache[pool_type]
            index          = cache["index"]
            descriptions   = cache["descriptions"]
            definitions    = cache["definitions"]
            strategy_names = cache["strategy_names"]

            k_search = min(len(descriptions), n * 5)
            if k_search == 0:
                continue
            _, indices = index.search(q_matrix, k_search)  # [batch_size, k_search]
            ...
            for b in range(batch_size):
                ...
                for idx in indices[b]:
                    ...
                    selected.append({
                        "Pool":       pool_type,
                        "Strategy":   s_name,
                        "Definition": definitions[idx],    # example-level definition
                        "Example":    descriptions[idx],   # the actual description text
                    })
                    if len(selected) >= n:
                        break
                results[b].extend(selected)
...
- Do NOT fabricate strategies not evidenced in the description."""
    ...
    def _run_summarizer(self, batch: DataProto, reward_extra_infos_dict: dict):
        ...
        """
        if not self.use_strategy or not self._strategy_embedding_model:
            return
        ...
        for i in range(len(batch)):
            ...
            query = f"{attack_target} {category}"
            summarizer_inputs.append((i, pool_type, query, malicious_desc, summarizer_prompt))
        ...
        for j, (orig_idx, pool_type, query, malicious_desc, _) in enumerate(summarizer_inputs):
            try:
            ...
            # ── store into the correct pool ──
            # structure: pool[strategy_name]["examples"] is a list of
            # {"description": str, "definition": str, "embedding": np.ndarray}
            pool = self.library[pool_type]
            embedding = self._strategy_embedding_model.encode(malicious_desc)
            ...
            if strategy_name not in pool:
                ...
                pool[strategy_name]["examples"].append(new_example)
                # sliding window: keep only most recent max_examples_per_strategy
                max_ex = self.config.trainer.get("max_examples_per_strategy", 5)
                if len(pool[strategy_name]["examples"]) > max_ex:
                    pool[strategy_name]["examples"].pop(0)

            # per-pool pruning: drop oldest strategy if pool exceeds pool_max_size
        ...
        sizes = {pt: len(p) for pt, p in self.library.items()}
        print(f"[Strategy] Pool sizes: {sizes}")

        # ── invalidate FAISS cache so it gets rebuilt on next retrieval ──
    ...
    def _init_training(self):
        ...
        print(f"[Init] Strategy library: { {pt: len(p) for pt, p in self.library.items()} }")
    ...
    def build_attack_prompt_batch(self, data: DataProto, is_val: bool = False) -> DataProto:
        ...
        Uses batch FAISS retrieval for efficiency (single index build per pool per call).
        """
        max_length = data.batch["input_ids"].shape[1]
        batch_size = data.batch["input_ids"].shape[0]

        # ── batch encode all queries at once (much faster than one-by-one) ──
        # use system_prompt + user_content as query so retrieval finds
        # strategies relevant to the actual task description
        all_strategy_lists = [[] for _ in range(batch_size)]
        if self.use_strategy and self._strategy_embedding_model:
            # queries = []
            # for i in range(batch_size):
            #     extra_info = data.non_tensor_batch["extra_info"][i]
            #     # 1. use previous step's malicious_tool_description (best)
            #     prev_desc = extra_info.get("malicious_tool_description", "")
            #     if not prev_desc.strip():
            #         # 2. fallback to description_cache (populated by _init_training)
            #         idx = extra_info.get("index", None)
            #         prev_desc = self.description_cache.get(idx, "") if idx is not None else ""
            #     queries.append(prev_desc)

            # # single batch encode call
            # query_embeddings = self._strategy_embedding_model.encode(
            #     queries, batch_size=64, show_progress_bar=False
            # )
            # # single FAISS index build per pool
            # all_strategy_lists = self._retrieve_strategies_batch(query_embeddings, is_val=is_val)
            ...
            for i in range(batch_size):
                ...
                cache = self.strategy_result_cache.get(idx, None)

                # 是否需要重新 retrieval
                if (
                    idx is None
                    or cache is None
                    or cache["library_version"] != self._library_version
                    or cache["desc"] != prev_desc
                ):
                    need_recompute.append(i)
                    recompute_queries.append(prev_desc)
                    recompute_indices.append(idx)
                else:
                    all_strategy_lists[i] = cache["strategies"]

            # 🔹 只对需要的样本做 encode + retrieval
            if recompute_queries:
                query_embeddings = self._strategy_embedding_model.encode(
                    recompute_queries,
                    batch_size=256,
                ...
                new_strategy_lists = self._retrieve_strategies_batch(query_embeddings, is_val=is_val)

                for j, i in enumerate(need_recompute):
                    idx = recompute_indices[j]
                    desc = recompute_queries[j]
                    strategies = new_strategy_lists[j]

                    all_strategy_lists[i] = strategies

                    if idx is not None:
                        self.strategy_result_cache[idx] = {
                            "library_version": self._library_version,
                            "desc": desc,
        ...
        for i in range(batch_size):
            ...
            strategy_list = all_strategy_lists[i]
    ...
    def fit(self):
        ...
        #     self._init_training()
        if self.use_strategy and len(self.description_cache) == 0:
            self._init_training()
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, "yellow"):
                        ...
                        target_input_batch = self.build_target_batch(gen_batch_output, new_batch)
                        for i in range(len(new_batch)):
                            extra_info = new_batch.non_tensor_batch["extra_info"][i]
                            idx = extra_info.get("index", None)
                            desc = extra_info.get("malicious_tool_description", "")
                            if idx is not None and desc:
                                self.description_cache[idx] = desc
                        ...
                        # ── 更新 description history（按 attack_target+category 分组）──
                        for i in range(len(new_batch)):
                            item = new_batch[i]
                            extra_info = item.non_tensor_batch.get("extra_info", {})
                            desc = extra_info.get("malicious_tool_description", "")
                        ...
                        if reward_extra_infos_dict:
                            ...
                            if self.use_strategy:
                                metrics["train/strategy_library_size"] = sum(len(p) for p in self.library.values())
